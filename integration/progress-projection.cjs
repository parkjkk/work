...
function projectProgress(state,snapshot,previous,now=new Date().toISOString(),planning){
 ...
 const linked=new Map();for(const [month,value]of Object.entries(state.months||{}))for(const row of value.rows||[]){const si=row.sourceIntegration;if(row.deleted||si?.repo!==snapshot.repo)continue;const key=si.path+'\0'+si.id;if(!linked.has(key))linked.set(key,[]);linked.get(key).push({month,row});}
 const indexes=new Map(),entries=[],unsafeJobs=new Set(),unlinkedProducts=new Set(),ix=productIndex(state.products);
 for(const [key,source]of raw){if(linked.has(key)||source.row.off||source.file.date>asOf)continue;const r=source.row,products=new Set([ix.ids.get(r.productId),ix.names.get(String(r.product||'').trim())].filter(Boolean));for(const product of products)unlinkedProducts.add(String(r.worker||'').trim()+'\0'+product.name);}
 ...
 for(const [key,links]of linked){if(links.length!==1)continue;const source=raw.get(key),{month,row}=links[0];if(!source||row.sourceIntegration.sourceHash!==hash(source.row))continue;
  const progress=progressFor(links[0]);if(!progress||unsafeJobs.has(jobKey(progress,month))||unlinkedProducts.has(String(row.worker||'').trim()+'\0'+row.product))continue;
  const number=value=>typeof value==='number'&&Number.isFinite(value)?value:null;
  const completionRow=progress.completionRowId&&Object.values(state.months).flatMap(m=>m.rows||[]).find(r=>r.id===progress.completionRowId);
