...
function projectProgress(state,snapshot,previous,now=new Date().toISOString(),planning){
 ...
 const revision=hash({asOf,entries,targets});if(previous?.schema===1&&previous.kind==='schedule-production-progress'&&previous.revision===revision&&hash({asOf:previous.asOf,entries:previous.entries,targets:previous.targets})===revision)return copy(previous);
 return{schema:1,kind:'schedule-production-progress',revision,updatedAt:date.toISOString(),asOf,entries,targets};
}
