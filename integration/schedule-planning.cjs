...
function computeJobs(s,k,context){const source=s.months[k],externalRows=Object.entries(s.months||{}).filter(([month,value])=>month<k&&!value.closed&&!value.closeSnapshot).flatMap(([,value])=>(value.rows||[]).filter(row=>!row.deleted&&scheduleTargetOf(row)?.month===k)),m={...source,rows:[...(source.rows||[]),...externalRows].map(r=>named(s,r)),plans:(source.plans||[]).map(r=>named(s,r)),jobs:(source.jobs||[]).map(r=>named(s,r))};if(m.closed&&m.plans?.length){return(m.jobs||[]).map(j=>{const p=m.plans.find(p=>p.source===j.source),current=p?.produced??0,previous=j.carried?(p?.previousProduced||0):0,total=(p?.plan||j.plan||0)+previous;return{...j,records:[],currentProduced:current,produced:current+previous,totalPlan:total,remaining:Math.max(0,total-current-previous),complete:current+previous>=total,archived:true}})}if(m.modern===false)return[];
