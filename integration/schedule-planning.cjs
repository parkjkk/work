...
const endsProduction=r=>!C.fieldPreparation(r)&&(r.plan===0||!!completion(r));
...
function scheduleTargetOf(row){try{return C.scheduleTargetValue(row?.scheduleTarget)}catch{return null}}
function scheduleTargetMatches(job,row,s=null){
 ...
 // A selected, exact manual reservation may start before its forecast date.
 // Only a positive actual supplies that evidence; preparation and zero do not.
 const rowMonth=C.iso(row.date)?row.date.slice(0,7):'',directEarlier=!!rowMonth&&t.month>rowMonth,earlyActual=!!job.manual&&!job.carried&&C.num(row.pours)>0;
 return !!t&&row.fieldPlanIntent!=='start'&&row.planIntent!=='start'&&t.worker===job.worker&&t.originId===(job.originId||job.id)&&(!productId||t.productId===productId)&&(s?C.catalogName(s,row.product)===C.catalogName(s,job.product):row.product===job.product)&&(!row.jobId||row.jobId===job.id)&&C.iso(row.date)&&(row.date>=productionStartDate(job)||earlyActual)&&(!directEarlier||job.manual&&t.jobId===job.id&&C.iso(job.start));
 }catch{return false}
}
function recordMatchesJob(job,row,s=null){
 if(!row.scheduleTarget)return (s?C.catalogName(s,row.product)===C.catalogName(s,job.product):row.product===job.product)&&row.worker===workerAt(job,row.date);
 return scheduleTargetMatches(job,row,s)&&(resolvedScheduleTargets.get(row)===job.id||(job.records||[]).some(r=>r.id===row.id&&r.date===row.date&&r.worker===row.worker&&JSON.stringify(r.scheduleTarget)===JSON.stringify(row.scheduleTarget)));
}
...
function resolveScheduleTargets(s,k,m,out,context){
 ...
 const months={};for(const [month,value]of Object.entries(s.months||{})){const rows=(value.rows||[]).filter(row=>month===k?!row.scheduleTarget:!(month<k&&scheduleTargetOf(row)?.month===k));months[month]={...value,rows}}const basis={...s,months},baseline=computeJobs(basis,k,context),accepted=new Map();
 const baselines=new Map();for(const j of baseline){if(!baselines.has(j.id))baselines.set(j.id,[]);baselines.get(j.id).push(j)}
 // Seed the start from independently linked actuals. A positive manual-target
 // row may establish an earlier start only after the identity and terminal
 // checks below. These are transient computed jobs; saved dates stay intact.
 ...
 for(const row of [...m.rows].sort((a,b)=>(a.date||'').localeCompare(b.date||''))){if(!row.scheduleTarget||row.deleted)continue;
  ...
  const candidates=out.filter(j=>scheduleTargetMatches(j,row)&&(!j.stoppedAt||row.date<=j.stoppedAt));if(candidates.length!==1)continue;
  const job=candidates[0],original=s.months?.[t.month];if(!original)continue;
  const originals=(original.jobs||[]).filter(j=>j.id===t.jobId&&j.worker===t.worker&&(j.originId||j.id)===t.originId&&C.catalogLookup(s).product(j.product)?.id===t.productId);
  ...
  if(began){resolvedScheduleTargets.set(row,job.id);if(C.num(row.pours)>0&&(!job.firstActual||row.date<job.firstActual))job.firstActual=row.date;if(!accepted.has(job.id))accepted.set(job.id,[]);accepted.get(job.id).push(row)}
 }
}

// A production job owns quantities and machines; assignments own dated labour.
// Never rewrite the worker on historical daily rows when an assignment changes.
function normalizedHandoffs(job){
 if(job.handoffs==null)return[];
 if(!Array.isArray(job.handoffs)||job.handoffs.length>100)throw Error('작업 넘기기 기록 형식을 확인해 주세요.');
 let worker=job.worker,date='',ids=new Set();
 return job.handoffs.map(h=>{if(!h||typeof h.id!=='string'||!h.id||ids.has(h.id)||!C.iso(h.date)||h.date<=date||C.iso(job.start)&&h.date<job.start&&!job.carried||h.from!==worker||typeof h.to!=='string'||!h.to.trim()||h.to===worker||h.daily!=null&&(!(Number.isFinite(h.daily)&&h.daily>0)||job.routing?.mode==='parallel')||h.otherWork&&(!String(h.otherWork.label||'').trim()||!C.iso(h.otherWork.end)||h.otherWork.end<h.date))throw Error('작업 넘기기 날짜·담당자 연결·하루 조수를 확인해 주세요.');if(h.quantity!=null){const q=h.quantity;if(!q||q.schema!==1||!Number.isFinite(q.recipientQty)||q.recipientQty<=0||!Number.isFinite(q.threshold)||q.threshold<0||!C.iso(q.sourceLastDate)||q.sourceLastDate>=h.date||!Number.isFinite(q.sourceLastQty)||q.sourceLastQty<0||job.routing?.mode==='parallel'||h.otherWork)throw Error('수량으로 나눈 작업의 배정 조수·마지막 생산일을 확인해 주세요.')}ids.add(h.id);date=h.date;worker=h.to;return h});
}
function safeHandoffs(job){try{return normalizedHandoffs(job)}catch{return[]}}
function workerAt(job,date){let worker=job.worker;for(const h of safeHandoffs(job)){if(h.date>date)break;worker=h.to}return worker}
function dailyAt(job,date,machine=null){let rate=machine?.daily??job.daily;const assignments=safeHandoffs(job);for(const h of assignments){if(h.date>date)break;if(h.daily>0){if(!(machine&&job.routing?.mode==='parallel'))rate=h.daily}}if(assignments.some(h=>h.quantity&&date>h.quantity.sourceLastDate&&date<h.date))return 0;const final=assignments.find(h=>h.quantity?.sourceLastDate===date&&h.quantity.sourceLastQty>0);return final?Math.min(rate,final.quantity.sourceLastQty):rate}
function handoffSegments(job,start,end){
 if(!C.iso(start)||!C.iso(end)||end<start)return[];const list=normalizedHandoffs(job),out=[];let worker=job.worker,begin=start,id='origin',incoming=null;
 for(const h of list){if(h.date<=start){worker=h.to;id=h.id;incoming=h;continue}if(h.date>end)break;const last=h.quantity?([h.quantity.sourceLastDate,add(h.date,-1)].sort()[0]):add(h.date,-1);if(begin<=last)out.push({worker,start:begin,end:last,assignmentId:id,handoffIn:incoming,handoffOut:h});worker=h.to;begin=h.date;id=h.id;incoming=h}
 out.push({worker,start:begin,end,assignmentId:id,handoffIn:incoming,handoffOut:null});return out;
}
function handoffRecordMatches(job,row){try{return recordMatchesJob(job,row)}catch{return !row.scheduleTarget&&row.worker===job.worker&&row.product===job.product}}
function otherWorkPeriods(s,start='1900-01-01',end='2200-12-31'){
 const values=new Map();for(const [month,m]of Object.entries(s.months||{}).sort(([a],[b])=>a.localeCompare(b)))for(const j of m.jobs||[])for(const h of Array.isArray(j.handoffs)?j.handoffs:[]){if(!h?.otherWork||!C.iso(h.date)||!C.iso(h.otherWork.end)||h.otherWork.end<h.date||h.date>end||h.otherWork.end<start||!h.from)continue;if(!safeHandoffs(j).includes(h)||(s.workers||[]).length&&(!(s.workers||[]).includes(h.from)||!(s.workers||[]).includes(h.to)))continue;const id='other-work:'+h.id;values.set(id,{id,jobId:j.id,worker:h.from,start:h.date,end:h.otherWork.end,label:String(h.otherWork.label||'다른 업무'),source:'handoff',handoffId:h.id,ownerMonth:month})}return [...values.values()];
}
const assignmentCalendarScopes=new WeakMap();
function withAssignmentCalendar(s,read){if(assignmentCalendarScopes.has(s))return read();const periods=otherWorkPeriods(s);assignmentCalendarScopes.set(s,periods);try{return read()}finally{assignmentCalendarScopes.delete(s)}}
function otherWorkOn(s,worker,date){return(assignmentCalendarScopes.get(s)||otherWorkPeriods(s,date,date)).some(r=>r.worker===worker&&r.start<=date&&r.end>=date)}
function creditedWork(s,worker,date){
...
function prepareHandoff(s,k,command){
 ...
 const selected=[job];if(command.swapJobId){const swap=computed.find(j=>j.id===command.swapJobId);if(!swap||swap.id===job.id||workerAt(swap,date)!==to)throw Error('같은 월에서 받을 작업자가 진행하는 맞교환 작업을 선택해 주세요.');selected.push(swap)}
 const preview=schedule(s,k,date),operationsId=command.operationId||C.id(),changes=[];
 for(const [index,j]of selected.entries()){
  const source=month.jobs?.find(v=>v.id===j.id),history=normalizedHandoffs(j);if(!index&&command.daily!=null&&j.routing?.mode==='parallel')throw Error('동시생산은 기존 호기별 하루 조수를 유지합니다. 호기별 속도는 작업조건에서 수정해 주세요.');if(j.complete||j.stopped||j.carryBlocked||j.handoffBlocked||!j.firstActual&&!(j.produced>0))throw Error('실적이 있는 진행 중 생산 작업만 넘길 수 있습니다.');
  if(date<j.start||history.some(h=>h.date>=date))throw Error('이전 작업 넘기기 이후의 날짜를 선택해 주세요.');
  if(Object.entries(s.months).some(([mk,m])=>mk>k&&(m.jobs||[]).some(v=>v.carried&&(v.originId||v.id)===(j.originId||j.id))))throw Error('다음달로 이월된 작업은 가장 최근 이월 월에서 넘겨 주세요.');
  const event={id:operationsId+':'+index,date,from:workerAt(j,date),to:index?from:to,...(!index&&command.daily!=null?{daily:Number(command.daily)}:{})};
  if(!index&&command.otherWork){if(selected.length>1)throw Error('맞교환과 다른 업무 기간은 함께 지정할 수 없습니다.');event.otherWork={label:String(command.otherWork.label||'').trim(),end:command.otherWork.end}}
  const expectedEnd=preview.rows.filter(r=>r.jobId===j.id&&!r.actualOnly&&!r.reservationOnly).map(r=>r.physicalEnd||r.end).filter(C.iso).sort().at(-1);if(expectedEnd&&date>expectedEnd)throw Error('현재 예상 생산은 '+expectedEnd.slice(5).replace('-','/')+'에 끝납니다. 그날 이전을 선택하거나 하루 조수를 확인해 주세요.');
  const next={...(source?clone(source):clone(j)),handoffs:[...clone(history),event]};normalizedHandoffs(next);
  for(const field of['records','produced','remaining','currentProduced','firstActual','lastActual','complete','totalPlan','unproduced','fieldPlanNotes','handoffReview','handoffBlocked'])if(!source)delete next[field];
  const ambiguous=(month.rows||[]).find(r=>!r.deleted&&r.date>=date&&r.worker===event.to&&C.catalogName(s,r.product)===j.product&&(C.num(r.pours)>0||endsProduction(r))&&r.jobId!==j.id);if(ambiguous)throw Error(ambiguous.date+'에 받을 작업자의 같은 품명 실적이 있습니다. 다른 생산과 섞이지 않도록 실적 연결을 먼저 확인해 주세요.');
  const contradictory=(j.records||[]).filter(r=>r.date>=date&&!r.deleted&&(C.num(r.pours)>0||endsProduction(r))&&!handoffRecordMatches(next,r));if(contradictory.length)throw Error(contradictory[0].date+'에 기존 담당자 실적이 있습니다. 원장을 옮기지 않으며 실적 다음 날짜부터 넘겨 주세요.');
  if(otherWorkOn(s,event.to,date))throw Error('받을 작업자에게 다른 업무 기간이 있습니다. 겹치는 기간을 먼저 확인해 주세요.');
 ...
 const conflicts=[...latest.values()].filter(j=>!selected.some(x=>(x.originId||x.id)===(j.originId||j.id)&&x.product===j.product)&&!j.complete&&!j.stopped&&(j.firstActual||j.produced>0)&&changes.some(x=>workerAt(j,date)===workerAt(x.job,date))).map(j=>({jobId:j.id,worker:workerAt(j,date),product:j.product,start:j.firstActual||j.start,end:j.end||null}));
...
function quotaConsumed(s,k,job,event,context=null){
 const used=j=>(j.records||[]).some(row=>row.date>=event.date&&(C.num(row.pours)>0||endsProduction(row)));
 if(used(job))return true;
 if(job.carried)for(const month of Object.keys(s.months||{}).filter(month=>month<k).sort()){
  const m=s.months[month],values=m.closed&&m.closeSnapshot?.schedule?.jobs||jobs(s,month,context||{jobs:new Map()});
  for(const prior of values)if((prior.originId||prior.id)===(job.originId||job.id)&&prior.worker===job.worker&&C.catalogName(s,prior.product)===job.product&&used(prior))return true;
 }
 // A frozen carry total above the quota boundary proves the receiver's share
 // is already in progress even when old individual records are unavailable.
 return !!(job.carried&&event.quantity&&event.date<k+'-01'&&(job.effectivePreviousProduced??job.previousProduced??0)>event.quantity.threshold);
}
function quotaWithoutEvent(s,k,job,event){
 if(!event)return{state:s,job};
 const origin=job.originId||job.id,strip=values=>(values||[]).map(value=>(value.originId||value.id)===origin&&value.worker===job.worker&&C.catalogName(s,value.product)===job.product?{...value,handoffs:(value.handoffs||[]).filter(h=>h.id!==event.id)}:value),months={};
 for(const [month,m]of Object.entries(s.months)){const snapshot=m.closeSnapshot?.schedule;months[month]={...m,jobs:strip(m.jobs),...(snapshot?{closeSnapshot:{...m.closeSnapshot,schedule:{...snapshot,jobs:strip(snapshot.jobs)}}}:{})}}
 const basis={...s,months},matches=jobs(basis,k,{jobs:new Map(),skipQuantityRefresh:true}).filter(value=>value.id===job.id);
 if(matches.length!==1)throw Error('수량을 나눌 원래 작업 연결을 확인해 주세요.');return{state:basis,job:matches[0]};
}
function quotaBoundary(s,k,job,to,threshold){
 const today=asOf(s,k),produced=job.produced,totalPlan=job.totalPlan;
 if(!Number.isFinite(produced)||!Number.isFinite(totalPlan)||!Number.isFinite(threshold)||threshold<0||threshold>totalPlan)throw Error('현재 누적과 계획 조수를 확인한 뒤 나눠 주세요.');
 let remaining=quotaRound(Math.max(0,threshold-produced)),sourceLastDate=today,sourceLastQty=0;
 for(let date=add(today,1),guard=0;remaining>0&&guard<740;date=add(date,1),guard++){
  if(!manualWork(s,workerAt(job,date),date))continue;const capacity=dailyAt(job,date);if(!(Number.isFinite(capacity)&&capacity>0))throw Error('기존 담당자의 하루 조수를 먼저 입력해 주세요.');sourceLastQty=quotaRound(Math.min(remaining,capacity));remaining=quotaRound(remaining-sourceLastQty);sourceLastDate=date;
 }
 if(remaining>0)throw Error('2년 안에 기존 담당자의 남은 수량을 생산할 수 없습니다.');
 const date=nextManualWork(s,to,add(sourceLastDate,1));return{date,sourceLastDate,sourceLastQty,totalPlan,produced,remaining:quotaRound(Math.max(0,totalPlan-produced)),retainedQty:quotaRound(Math.max(0,threshold-produced)),recipientQty:quotaRound(Math.max(0,totalPlan-Math.max(produced,threshold))),threshold};
}
function quotaPriorShare(s,k,job,event){
 if(event.quantity)return quotaRound(Math.min(event.quantity.recipientQty,Math.max(0,job.totalPlan-job.produced)));
 let produced=job.produced;const today=asOf(s,k);
 for(let date=add(today,1),guard=0;date<event.date&&guard<740&&produced<job.totalPlan;date=add(date,1),guard++){if(!manualWork(s,workerAt(job,date),date))continue;const rate=dailyAt(job,date);if(!(Number.isFinite(rate)&&rate>0))throw Error('기존 날짜 예약의 배정량을 계산하려면 하루 조수가 필요합니다.');produced=Math.min(job.totalPlan,produced+rate)}
 return quotaRound(Math.max(0,job.totalPlan-produced));
}
function prepareQuantityHandoff(s,k,command){
 ...
 if(base.complete||base.stopped||base.carryBlocked||base.handoffBlocked)throw Error('원래 작업의 남은 수량과 실적 연결을 확인해 주세요.');
 const remaining=quotaRound(Math.max(0,base.totalPlan-base.produced)),previousRecipientQty=pending?quotaPriorShare(baseline.state,k,base,pending):0,adding=command.addQuantity!=null;
 if(adding&&!pending)throw Error('추가 배정은 같은 받을 작업자의 아직 시작하지 않은 넘기기 예약에서 사용해 주세요.');
 const delta=adding?Number(command.addQuantity):0,recipientQty=quotaRound(adding?previousRecipientQty+delta:Number(command.quantity));
 if(adding&&(!Number.isFinite(delta)||delta<=0)||!Number.isFinite(recipientQty)||recipientQty<=0||recipientQty>remaining+0.000001)throw Error('받을 수량은 현재 남은 '+remaining+'조 안에서 입력해 주세요.');
 if(command.daily!=null&&!(Number.isFinite(Number(command.daily))&&Number(command.daily)>0))throw Error('받을 작업자의 하루 조수는 0보다 크게 입력해 주세요.');
 ...
 const conflicts=[...latest.values()].filter(other=>(other.originId||other.id)!==(job.originId||job.id)&&!other.complete&&!other.stopped&&(other.firstActual||other.produced>0)&&workerAt(other,event.date)===to).map(other=>({jobId:other.id,worker:to,product:other.product,start:other.firstActual||other.start,end:other.end||null}));
 if(conflicts.length&&!command.allowConcurrent){const error=Error('받을 작업자에게 진행 중 생산이 있습니다. 동시생산 여부를 확인해 주세요.');error.code='handoff-concurrent';error.conflicts=conflicts;throw error}
 return{operationId,date:event.date,from,to,changes:[{month:k,job:next}],conflicts,policy:'quantity-assignment-v1',quantityPlan:{...boundary,recipientQty,previousRecipientQty,addedQty:adding?delta:quotaRound(recipientQty-previousRecipientQty),pendingId:pending?.id||null,amended:!!pending}};
}
function refreshQuantityHandoffs(s,k,out,context){
 if(context?.skipQuantityRefresh)return out;const changes=new Map(),failures=new Map();
 for(const job of out){if(job.handoffBlocked||job.carryBlocked)continue;const quota=safeHandoffs(job).filter(h=>h.quantity);if(!quota.length)continue;
  try{const list=normalizedHandoffs(job);for(const event of quota){if(quotaConsumed(s,k,job,event,context))continue;if(event!==list.at(-1))throw Error('아직 시작하지 않은 수량 인계 뒤에 다른 넘기기가 있습니다. 이후 넘기기 순서를 먼저 확인해 주세요.');const baseline=quotaWithoutEvent(s,k,job,event),base=baseline.job;if(base.routing?.mode==='parallel')throw Error('수량 인계가 있는 작업은 단일·순차생산으로 유지해 주세요.');if(Math.abs(base.totalPlan-event.quantity.threshold-event.quantity.recipientQty)>0.000001)throw Error('계획 조수가 바뀌었습니다. 나눠 맡을 수량을 다시 확인해 주세요.');const boundary=quotaBoundary(baseline.state,k,base,event.to,event.quantity.threshold),updated={...clone(event),date:boundary.date,quantity:{...clone(event.quantity),sourceLastDate:boundary.sourceLastDate,sourceLastQty:boundary.sourceLastQty}};if(JSON.stringify(updated)!==JSON.stringify(event))changes.set(job.id,{...job,handoffs:list.map(h=>h.id===event.id?updated:clone(h))});}}
  catch(error){failures.set(job.id,error.message);job.handoffBlocked=true;job.handoffReview=error.message}
 }
 if(!changes.size)return out;
 const m=s.months[k],stored=[...(m.jobs||[])];for(const [id,job]of changes){const index=stored.findIndex(j=>j.id===id),original=index<0?job:stored[index],updated={...original,handoffs:clone(job.handoffs)};if(index<0)stored.push(updated);else stored[index]=updated}
 const basis={...s,months:{...s.months,[k]:{...m,jobs:stored}}},result=computeJobs(basis,k,{jobs:new Map(),skipQuantityRefresh:true});for(const job of result)if(failures.has(job.id)){job.handoffBlocked=true;job.handoffReview=failures.get(job.id)}return result;
}
function splitHandoffRows(rows,jobs){return rows.flatMap(row=>{const j=jobs.find(j=>j.id===row.jobId);if(!j?.handoffs?.length||j.handoffBlocked)return[row];return handoffSegments(j,row.start,row.end).map(segment=>({...row,id:row.id+':assignment:'+segment.assignmentId,physicalRowId:row.id,physicalStart:row.start,physicalEnd:row.end,worker:segment.worker,start:segment.start,end:segment.end,groupStart:segment.start,groupEnd:segment.end,assignmentId:segment.assignmentId,assignmentStart:segment.start,assignmentEnd:segment.end,handoffIn:segment.handoffIn,handoffOut:segment.handoffOut,dry:segment.end===row.end?row.dry:null,physicalDry:row.dry,handoffSegment:true,...(segment.handoffOut?{status:'작업 넘김',projected:false}:{})}))})}
...
function sourceContinuationLinks(s,m,stored){
 ...
 for(const r of m.rows||[]){if(r.deleted||r.jobId||!(startPlan(r)>0)||r.fieldPlanIntent==='start')continue;const matches=stored.filter(j=>{if(!j.handoffs?.length||!handoffRecordMatches(j,r)||!safeHandoffs(j).some(h=>h.date<=r.date)||j.stoppedAt||j.complete||j.ended)return false;const prior=m.rows.filter(x=>!x.deleted&&x.id!==r.id&&C.iso(x.date)&&(x.date<r.date||x.date===r.date&&m.rows.indexOf(x)<m.rows.indexOf(r))&&x.date>=(j.previousStart||j.start)&&handoffRecordMatches(j,x)&&(!x.jobId||x.jobId===j.id));if(prior.some(x=>endsProduction(x)||startPlan(x)>0&&x.fieldPlanIntent==='start'&&x.id!==j.sourceRowId))return false;const produced=(j.carried?j.previousProduced||0:0)+C.sum(prior,x=>x.pours),target=(j.carried?j.previousProduced||0:0)+(j.plan||0);return !(target>0&&produced>=target)});if(matches.length===1){const j=matches[0];links.set(r.id,{kind:'handoff-continue',jobId:j.id,sourceRowId:j.sourceRowId||null,date:r.date,rowId:r.id,sourcePlan:startPlan(r),originalPlan:j.plan,message:'작업 넘기기로 이어받은 같은 생산 작업의 누적입니다. 기존 계획과 호기 배정을 유지합니다.'})}}
...
function reservationContext(s,k,normalized=null,knownContinuations=null){
 const source=s.months?.[k],m=normalized||{...source,rows:(source?.rows||[]).filter(r=>!C.fieldPreparation(r)).map(r=>named(s,r)),jobs:(source?.jobs||[]).map(j=>named(s,j))},links=new Map(),candidates=new Map(),selectable=new Map(),issues=[],controlled=new Set();
 ...
 for(const r of ordered){
  if(r.scheduleTarget){controlled.add(r.id);continue}
  ...
  if(r.jobId&&explicitJob&&!handoffRecordMatches(explicitJob,r)){controlled.add(r.id);warn(r,'explicit-job-identity','연결된 작업과 현재 작업자·품명이 다릅니다. 이 실적은 이전 작업에 합산하지 않으며 일정·호기에서 연결을 확인해 주세요.',[r.jobId]);continue}
...
function applyFieldScheduleConditions(j){
 ...
 for(const r of j.records||[]){let v=r.fieldScheduleConditions;if(!v||v.schema!==1||!handoffRecordMatches(j,r))continue;if(r.scheduleTarget){v={schema:1,...Object.fromEntries(['machines','daily','routing'].filter(f=>v.edits?.[f]).map(f=>[f,v[f]]))};}
  const assigned=[...safeHandoffs(j)].reverse().find(h=>h.date<=r.date);if(assigned){if(Number.isFinite(v.daily)&&v.daily>0&&j.routing?.mode!=='parallel'){source.handoffDaily??={};const revision=r.sourceIntegration?.scheduleRevisions?.daily||0,prior=source.handoffDaily[assigned.id],sameValue=prior&&prior.value===v.daily&&prior.rowId!==r.id&&!revision,proof=sameValue?prior:{rowId:r.id,signature:JSON.stringify([r.id,v.daily,revision]),value:v.daily};source.handoffDaily[assigned.id]=proof;if(!fieldScheduleOverrideMatches(j.fieldScheduleOverrides?.['handoffDaily:'+assigned.id],proof.signature)){j.handoffs=clone(j.handoffs);j.handoffs.find(x=>x.id===assigned.id).daily=v.daily;}}const machineKey=a=>JSON.stringify((a||[]).map(m=>({name:routingMachineName(m.name),qty:C.num(m.qty),daily:C.num(m.daily)}))),routeKey=value=>{if(!value)return'';const result={...value};delete result.barDisplay;return JSON.stringify(result)};if(Array.isArray(v.machines)&&v.machines.length&&machineKey(v.machines)!==machineKey(machines)||v.routing&&routeKey(v.routing)!==routeKey(source.routing?.value||j.routing))j.handoffReview='인계 후 일보의 호기·생산 순서 변경은 적용하지 않았습니다. 인계 당시 호기를 유지하며 실제 호기별 생산량은 그대로 반영합니다.';continue;}
...
function captureFieldScheduleOverrides(s,k,record,cachedJobs=null){
 ...
 for(const h of record.handoffs||[]){const proof=before.fieldScheduleSource?.handoffDaily?.[h.id],old=before.handoffs?.find(x=>x.id===h.id);if(proof&&old&&h.daily!==old.daily)record.fieldScheduleOverrides={...(record.fieldScheduleOverrides||{}),['handoffDaily:'+h.id]:proof.signature}}
...
function reconcileCarry(s,k,j,context){
 ...
 const keys=Object.keys(s.months||{}).filter(key=>key<k).sort().reverse();let previous=null,sourceMonth=null;
 for(const key of keys){const month=s.months[key],values=month.closed&&month.closeSnapshot?.schedule?.jobs?month.closeSnapshot.schedule.jobs:jobs(s,key,context),matches=values.filter(p=>(p.originId||p.id)===j.originId&&p.worker===j.worker&&C.catalogName(s,p.product)===j.product);if(matches.length>1){j.carryReview='같은 이월 작업의 이전 원장이 중복되어 누적을 확인할 수 없습니다. 원월의 작업 연결을 확인해 주세요.';j.carryBlocked=true;break}if(matches.length===1){previous=matches[0];sourceMonth=key;break}if(j.carryFrom===key&&month.jobs?.some(p=>(p.originId||p.id)===j.originId)){j.carryReview='이월 원장의 작업자·품명이 달라졌습니다. 원월과 현재 월의 작업 연결을 확인해 주세요.';j.carryBlocked=true;break}}
 if(previous?.carryBlocked){j.carryReview=previous.carryReview;j.carryBlocked=true}
 if(j.carryBlocked){j.produced=null;j.remaining=null;j.unproduced=null;j.complete=false;return}
 if(!previous){
  const revokedStart=keys.some(key=>{const month=s.months[key],matches=[...(month.rows||[]),...(month.history||[]).map(event=>event.before).filter(Boolean)];return matches.some(row=>'job:'+row.id===j.originId&&row.fieldPlannedWorkRef&&row.worker===j.worker&&C.catalogName(s,row.product)===j.product)});
  if(revokedStart){j.carryReview='예정작업의 최초 생산이 취소되거나 삭제되어 이월 연결을 다시 확인해야 합니다. 이후 실제 생산은 보존했습니다.';j.carryBlocked=true;j.produced=null;j.remaining=null;j.unproduced=null;j.complete=false;}
  return;
 }
 if(!Number.isFinite(previous.produced))return;
 if(previous.handoffBlocked){j.handoffBlocked=true;j.handoffReview=previous.handoffReview}if(safeHandoffs(previous).length){try{const current=normalizedHandoffs(j),events=new Map(previous.handoffs.map(h=>[h.id,clone(h)]));for(const h of current)events.set(h.id,h);j.handoffs=[...events.values()].sort((a,b)=>a.date.localeCompare(b.date))}catch(error){j.handoffBlocked=true;j.handoffReview=error.message}}
 const originalPlan=j.totalPlan;
 j.effectivePreviousProduced=previous.produced;
 if(previous.stopped||previous.ended||previous.complete&&previous.remaining===0){
  const message='이월 이전 작업이 '+(previous.stopped?'중단':'완료')+'되었습니다. 이후 실적은 보존했으며 새 작업 연결이 필요합니다.';
  j.carryUnlinkedRows=(j.records||[]).map(r=>r.id);j.carryReview=message;j.carryTerminal=true;j.records=[];j.currentProduced=0;j.produced=previous.produced;j.unproduced=Math.max(0,originalPlan-j.produced);j.remaining=0;j.complete=true;j.ended=!previous.stopped;j.stopped=!!previous.stopped;j.firstActual=previous.firstActual||previous.previousStart||previous.start;j.lastActual=previous.lastActual||previous.completedAt||previous.interruptedAt||null;
  for(const field of['completedAt','completionRowId','completionReason','completionStockQty','completionNote','interruptedAt'])j[field]=previous[field]??null;
  for(const field of['machines','daily','routing'])if(Object.prototype.hasOwnProperty.call(previous,field))j[field]=clone(previous[field]);
  return;
 }
 j.produced=previous.produced+j.currentProduced;j.unproduced=Math.max(0,originalPlan-j.produced);j.remaining=j.stopped||j.ended?0:j.unproduced;j.complete=j.stopped||j.ended||originalPlan>0&&j.remaining===0;
 if(j.complete&&!j.stopped){j.completedAt=j.completedAt||j.lastActual;j.completionReason||='target-reached'}
 else if(!j.stopped&&!j.ended){j.completedAt=null;j.completionReason=null}
 if(previous.produced!==j.previousProduced)j.carryReconciliation={sourceMonth,storedPreviousProduced:j.previousProduced,actualPreviousProduced:previous.produced};
}
// Several stale clients can submit a first actual on different dates. The explicit
// preparation reference identifies one physical job; normalized display rows keep
// every source plan/intent untouched while later starts join its first actual.
function plannedContinuationRows(s,k,rows,stored,context){
 if(!rows.some(row=>row.fieldPlannedWorkRef))return rows;
 const key=row=>{try{const ref=C.plannedWorkRefValue(row.fieldPlannedWorkRef);return ref?JSON.stringify([ref.date,ref.id,row.worker,C.catalogIdentity(s,row.product)]):null}catch{return null}};
 const cache=context&&(context.plannedSources||(context.plannedSources=new WeakMap()));let groups=cache?.get(s);
 if(!groups){groups=new Map();for(const [month,value]of Object.entries(s.months||{}))for(const row of value.rows||[]){if(row.deleted||C.fieldPreparation(row)||!(C.num(row.pours)>0))continue;const id=key(row);if(id){if(!groups.has(id))groups.set(id,[]);groups.get(id).push({month,row})}}for(const values of groups.values())values.sort((a,b)=>(a.row.date+'\0'+a.row.id).localeCompare(b.row.date+'\0'+b.row.id));cache?.set(s,groups);}

 return rows.map(row=>{
  const reference=key(row);if(!reference)return row;const first=groups.get(reference)?.[0];
  if(!first||row.date<first.row.date)return{...row,_plannedWaiting:true};
  if(first.row.id===row.id){if(!row.scheduleTarget&&!(startPlan(row)>0)&&row.fieldPlannedWorkPlan>0)return named(s,{...row,plan:row.fieldPlannedWorkPlan,fieldPlanIntent:'start'});return row;}
  if(!(C.num(row.pours)>=0)||row.productionCompletion||row.plan===0)return row;
  // An explicit different target remains protected by the normal target validator.
  if(first.row.scheduleTarget){if(row.scheduleTarget&&JSON.stringify(row.scheduleTarget)!==JSON.stringify(first.row.scheduleTarget))return row;return named(s,{...row,plan:null,fieldPlanIntent:'continue',scheduleTarget:clone(first.row.scheduleTarget)})}
  if(!(startPlan(first.row)>0||first.row.fieldPlannedWorkPlan>0)||row.scheduleTarget)return row;
  const origin='job:'+first.row.id,carried=stored.filter(j=>j.carried&&(j.originId||j.id)===origin&&j.worker===first.row.worker&&C.catalogIdentity(s,j.product)===C.catalogIdentity(s,row.product));
  const jobId=first.month===k?origin:carried.length===1?carried[0].id:origin;
  return named(s,{...row,plan:null,fieldPlanIntent:'continue',jobId});
 });
}
function computeJobs(s,k,context){const source=s.months[k],externalRows=source.closed||source.closeSnapshot?[]:Object.entries(s.months||{}).filter(([month,value])=>month<k&&!value.closed&&!value.closeSnapshot).flatMap(([,value])=>(value.rows||[]).filter(row=>!row.deleted&&scheduleTargetOf(row)?.month===k)),m={...source,rows:[...(source.rows||[]),...externalRows].filter(r=>!C.fieldPreparation(r)).map(r=>named(s,r)),plans:(source.plans||[]).map(r=>named(s,r)),jobs:(source.jobs||[]).map(r=>named(s,r))};if(m.closed&&m.plans?.length){return(m.jobs||[]).map(j=>{const p=m.plans.find(p=>p.source===j.source),current=p?.produced??0,previous=j.carried?(p?.previousProduced||0):0,total=(p?.plan||j.plan||0)+previous;return{...j,records:[],currentProduced:current,produced:current+previous,totalPlan:total,remaining:Math.max(0,total-current-previous),complete:current+previous>=total,archived:true}})}if(m.modern===false)return[];
 m.rows=plannedContinuationRows(s,k,m.rows,m.jobs||[],context).filter(row=>!row._plannedWaiting);
 const stored=m.jobs||[],continuations=sourceContinuationLinks(s,m,stored),reservations=reservationContext(s,k,m,continuations),anchors=new Map([...reservations.links.values()].map(x=>[x.jobId,x.rowId])),anchor=j=>j.sourceRowId||anchors.get(j.id),out=[],starts=m.rows.filter(r=>!r.deleted&&!r.scheduleTarget&&startPlan(r)>0&&!r.jobId&&!C.catalogIdentity(s,r.product).includes('부속')&&!continuations.has(r.id)&&!reservations.links.has(r.id)),occ={};
 for(const r of starts){const index=occ[r.product]||0;occ[r.product]=index+1;const prior=stored.find(j=>j.sourceRowId===r.id),identityChanged=prior&&(prior.worker!==r.worker||prior.product!==r.product),cfg=identityChanged?{}:prior||{},legacy=identityChanged||r.sourceIntegration&&!cfg.sourceRowId?null:m.plans.filter(p=>p.product===r.product)[index],parsed=parseInput(legacy?.input||'',k,s.workers);out.push({id:'job:'+r.id,originId:'job:'+r.id,sourceRowId:r.id,product:r.product,worker:r.worker,start:r.date,previousProduced:legacy?.previousProduced||0,previousPlan:legacy?.previousPlan||0,...parsed,...cfg,plan:startPlan(r),start:cfg.start||parsed.start||r.date,worker:cfg.worker||parsed.worker||r.worker})}
 ...
 resolveScheduleTargets(s,k,m,out,context);
 for(const j of out){
  ...
  if(startIndex>=0)for(let i=startIndex;i<ordered.length;i++){const r=ordered[i];if(!handoffRecordMatches(j,r))continue;const linked=reservations.links.get(r.id);if(i>startIndex&&!r.scheduleTarget&&startPlan(r)>0&&continuations.get(r.id)?.jobId!==j.id&&((linked&&linked.jobId!==j.id)||!r.jobId&&continuations.get(r.id)?.sourceRowId!==anchorId)){endIndex=i;nextStart=true;break}if(r.jobId&&r.jobId!==j.id)continue;if(endsProduction(r)){endIndex=i+1;break}}
  let records=ordered.filter((r,i)=>r.scheduleTarget?resolvedScheduleTargets.get(r)===j.id&&i<endIndex:r.jobId===j.id&&handoffRecordMatches(j,r)&&i<endIndex||(!r.jobId&&handoffRecordMatches(j,r)&&(!safeHandoffs(j).some(h=>r.date>=h.date)||!out.some(other=>other.id!==j.id&&handoffRecordMatches(other,r)&&(other.previousStart||other.start)<=r.date&&(!other.sourceRowId||ordered.findIndex(x=>x.id===other.sourceRowId)<=i)))&&(!reservations.links.has(r.id)||reservations.links.get(r.id).jobId===j.id)&&(startIndex>=0?i>=startIndex&&i<endIndex:j.sourceStart!=null?r.sourceRow>=j.sourceStart&&r.sourceRow<(j.sourceEnd||1000):!(j.manual&&!j.carried&&reservations.controlled.has(r.id))&&!out.some(x=>anchor(x)&&x.product===j.product&&x.worker===j.worker)&&(!j.start||r.date>=j.start))));
  if(startIndex<0&&!anchorId&&j.sourceStart==null){const siblings=out.filter(x=>!anchor(x)&&x.product===j.product&&x.worker===j.worker&&x.start<=j.start);if(siblings.length>1)records=records.filter(r=>r.jobId===j.id||resolvedScheduleTargets.get(r)===j.id)}
 ...
 for(const j of out){reconcileCarry(s,k,j,context);try{normalizedHandoffs(j);if(j.handoffs?.some(h=>!s.workers.includes(h.from)||!s.workers.includes(h.to)))throw Error('작업 넘기기 기록의 담당자가 관리 목록에 없습니다.')}catch(error){j.handoffBlocked=true;j.handoffReview=error.message}}
...
function dailyProgress(s,k,options={}){
 ...
 for(const [key,m]of Object.entries(working.months).sort(([a],[b])=>a.localeCompare(b))){
  ...
  const inferred=m.closed&&values.some(j=>!j.records?.length)?jobs({...working,months:{...working.months,[key]:{...m,closed:false}}},key):[];
   for(const j of values){const matches=inferred.filter(x=>x.id===j.id||j.sourceRowId&&x.sourceRowId===j.sourceRowId),frozenAggregate=!!m.closed&&!j.records?.length,rowLinks=j.records?.length?j.records:matches.length===1?matches[0].records:[],entry={key,job:j,rowLinks,frozenAggregate};all.push(entry);for(const r of rowLinks)if(selectedKeys.has(rowKey(r))){if(!owners.has(r.id))owners.set(r.id,[]);owners.get(r.id).push(entry)}}
 }
 ...
 function progress(entry){
  const j=entry.job,origin=j.originId||j.id,chainKey=origin+'\0'+j.worker+'\0'+j.product;if(calculations.has(chainKey))return calculations.get(chainKey);
   const chain=all.filter(e=>(e.job.originId||e.job.id)===origin&&e.job.worker===j.worker&&e.job.product===j.product&&(e.key<=cutoff.slice(0,7)||(e.rowLinks||[]).some(r=>!r.deleted&&C.iso(r.date)&&r.date<=cutoff&&selectedKeys.has(rowKey(r))))&&(!month.closed||e.key<=k)).sort((a,b)=>a.key.localeCompare(b.key));
  ...
  if(archiveUncertain||latest.carryBlocked||latest.handoffBlocked||new Set(chain.map(e=>e.key)).size!==chain.length||applicable.some(r=>invalidRows.has(r.id)))Object.assign(value,{produced:null,remaining:null,unproduced:null,percent:null,complete:false,warning:archiveUncertain?'마감된 작업의 해당 기준일까지 누적을 확인할 자료가 부족합니다.':'이 작업의 생산·계획 또는 완료 입력값을 확인해 주세요.'});
  if(latest.carryBlocked||latest.handoffBlocked)value.warning=latest.carryReview||latest.handoffReview;else if(latest.handoffReview)value.warning=[value.warning,latest.handoffReview].filter(Boolean).join(' ');
 ...
 for(const r of selected){if(r.deleted)continue;const linked=owners.get(r.id)||[],reservationWarning=reservationIssues.get(r.id)||'';if(linked.length===1){const v=progress(linked[0]),reservationLink=linked[0].job.fieldPlanNotes?.find(n=>n.kind==='reservation-start')||null;result.set(r.id,{...v,jobId:linked[0].job.id,...(r.scheduleTarget?{scheduleWorker:workerAt(linked[0].job,r.date)}:{}),reservationLink:reservationLink?clone(reservationLink):null,reservationWarning,warning:[v.warning,reservationWarning].filter(Boolean).join(' ')})}else result.set(r.id,{jobId:null,plan:null,produced:null,remaining:null,unproduced:null,percent:null,complete:false,completedAt:null,completionRowId:null,reason:null,stockQty:null,asOf:cutoff,reservationWarning,warning:reservationWarning||(r.scheduleTarget?'반영할 작업의 품명·시작/완료·이월 연결을 확인해 주세요. 실제 일보는 보존했습니다.':linked.length?'한 실적이 여러 작업에 연결되어 확인이 필요합니다.':'연결된 작업이 없습니다. 시작 계획 또는 작업 연결을 확인해 주세요.')})}
 for(const entry of all.filter(e=>e.key===k)){
  const j=entry.job,ids=j.carryBlocked?(entry.rowLinks||[]).map(r=>r.id):j.carryUnlinkedRows||[];
  for(const id of ids)if(result.has(id))result.set(id,{...result.get(id),jobId:null,produced:null,remaining:null,unproduced:null,percent:null,complete:false,completedAt:null,completionRowId:null,reason:null,warning:j.carryReview,reservationWarning:j.carryReview});
 }
 return result;
}
...
function manualDurationEnd(s,worker,start,duration){
 if(!C.iso(start)||!Number.isInteger(duration)||duration<1)throw Error('수기 시작일과 작업기간을 확인해 주세요.');
 for(let d=start,n=0,used=0;n<740;d=add(d,1),n++)if(manualWork(s,worker,d)&&++used>=duration)return d;
 throw Error('740일 안에 수기 생산을 마칠 수 없습니다.');
}
...
function ownedMachineEvents(s,k,j){
 let initial=j.carried?(j.effectivePreviousProduced??j.previousProduced??0):0,events=(j.records||[]).filter(r=>!r.deleted&&C.iso(r.date)&&r.pours>0).map(r=>({date:r.date,pours:r.pours,...(Array.isArray(r.machineActuals)?{machineActuals:clone(r.machineActuals)}:{})}));
...
function actualMachineSpans(s,k,j,alloc,today,independentSingle=false){
 // Each active product owns its completion date, even when one combined bar
 // contains other products. A single machine must not inherit the group's end.
...
function routingSchedule(s,k,input,today,statusToday){
 ...
 const key=j=>j.worker+'|routing:'+(j.handoffs?.length?'handoff:'+j.id:j.routing?.group||j.id);
 ...
 function calculate(j,group,predecessor){
  if(j.carryBlocked||j.handoffBlocked){observed(j,group,j.carryReview||j.handoffReview);return null}
  ...
  if(alloc.length===1&&alloc[0].daily==null&&j.daily>0)alloc[0].daily=j.daily;
  ...
  const active=j.produced>0||!!j.firstActual,planned=j.carried&&!manualUsesCalendar(j)&&j.start<k+'-01'?k+'-01':j.start;
  ...
  if(!j.complete){
   ...
   for(let d=first,n=0;!finished&&n<740;d=add(d,1),n++){
    if(!manualWork(s,workerAt(j,d),d))continue;
    let capacity=dailyAt(j,d);
    for(const v of stats){const left=v.machine.qty-v.forecast;if(left<=0)continue;const qty=Math.min(left,mode==='parallel'?dailyAt(j,d,v.machine):capacity);if(!(qty>0))continue;v.start||=d;v.forecast+=qty;v.end=d;if(mode==='sequential')capacity-=qty;}
...
// Excel's plan row follows one pacing product, never the sum of companions.
function pacingMember(members){const active=members.filter(j=>!j.complete);return[...(active.length?active:members)].sort((a,b)=>b.remaining/(b.daily||1)-a.remaining/(a.daily||1)||b.remaining-a.remaining||b.totalPlan-a.totalPlan)[0]}
function scheduleBase(s,k,requestedToday,options={}){const today=C.iso(options.asOf)?options.asOf:asOf(s,k),statusToday=C.iso(requestedToday)?requestedToday:today;if(s.months[k].closed&&s.months[k].closeSnapshot?.schedule)return clone(s.months[k].closeSnapshot.schedule);if(s.months[k].closed){const saved=history(s,k+'-01',add(add(k+'-01',32).slice(0,7)+'-01',-1));if(saved.length)return{jobs:jobs(s,k),rows:saved,warnings:[]};return{jobs:jobs(s,k),rows:(s.scheduleRows||[]).map(r=>({id:'archive:'+r.row,jobId:'',worker:r.cells[0],machine:r.cells[1],dry:C.serial(C.num(r.cells[3])),product:r.cells[4],qty:r.cells[5],start:C.serial(C.num(r.cells[7])),end:C.serial(C.num(r.cells[8])),status:r.cells[9],lot:r.cells[10],archived:true})).filter(r=>r.product&&r.start&&r.end&&r.start<=add(k+'-01',31)&&r.end>=k+'-01'),warnings:[]}}const raw=options.inputJobs||jobs(s,k),hasProduction=j=>!!j.firstActual||C.num(j.produced)>0||C.num(j.previousProduced)>0,all=raw.filter(j=>hasProduction(j)||!j.manual||j.manualSource==='timeline'||!raw.some(a=>!a.manual&&a.worker===j.worker&&a.product===j.product&&JSON.stringify(machineQty(a))===JSON.stringify(machineQty(j))&&a.start<=j.end)),output=[],warnings=[],groups=new Map(),groupIds=new Map();
 const baseKey=j=>(j.handoffs?.length?'handoff:'+j.id:j.worker)+'|'+(Number(j.group)>0||String(j.group).startsWith('manual:')?'group:'+j.group:'job:'+j.id),actualGroups=new Set(all.filter(hasProduction).map(baseKey));
 // Expected companions cannot lead, extend or hide a job that has real output.
 for(const j of all.filter(j=>!j.routing&&!j.carryBlocked&&!j.handoffBlocked)){const base=baseKey(j),key=base+(actualGroups.has(base)&&!hasProduction(j)?'|forecast':'');groupIds.set(j.id,key);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(j)}
 ...
 for(const j of all)if(j.carryBlocked||j.handoffBlocked){if(!j.routing)observed(j,j.carryReview||j.handoffReview)}else if(j.carryReview||j.handoffReview)warnings.push({id:j.id,product:j.product,message:j.carryReview||j.handoffReview});
 for(const members of list){try{const leader=pacingMember(members),w=leader.worker;const invalid=members.filter(j=>!j.worker||!j.start||!j.machines.length||!machineQty(j).some(a=>a.qty>0)||!j.complete&&!(j.daily>0)&&(hasProduction(j)||!j.duration&&!j.end));if(invalid.length){for(const j of invalid)warnings.push({id:j.id,product:j.product,message:'작업자·시작일·하루 조수·호기 배치를 확인해 주세요.'});for(const j of members)if(hasProduction(j))observed(j);continue}let start=leader.previousStart||leader.firstActual||leader.start,end=leader.lastActual||start,used=[];if(!C.iso(start))throw Error('유효한 시작일이 필요합니다.');if(start.slice(0,7)>k&&!leader.manual){warnings.push({id:leader.id,product:leader.product,message:'다음달 이후 시작 예정입니다.'});continue}
 ...
 else{let produced=leader.carried?(leader.effectivePreviousProduced??leader.previousProduced??0):0;const dailyActual={};for(const r of leader.records)dailyActual[r.date]=(dailyActual[r.date]||0)+(r.pours||0);let d=start;if(!hasProduction(leader)&&cursors[w]&&start<=cursors[w])d=add(cursors[w],1);for(let i=0;i<740;i++,d=add(d,1)){if(d<=today){produced+=dailyActual[d]||0;if(work(s,workerAt(leader,d),d,today,leader.start))used.push(d)}else if(work(s,workerAt(leader,d),d,today,leader.start)){used.push(d);produced+=dailyAt(leader,d)}if(produced>=leader.totalPlan){end=d;break}if(i===739)throw Error('2년 안에 목표수량을 채우지 못합니다.')}if(!hasProduction(leader))start=used[0]||start}
 ...
 const dry=reservationOnly?null:C.dryDate(segEnd,factory(s));output.push({...(pendingManual?{manualCalendarPolicy,manualWorkdays:manualDays.length,reservationOnly,fixedManualPeriod}:{}),...(actualMachineEndAdjusted?{actualMachineEndAdjusted:true}:{}),id:j.id+':'+i,jobId:j.id,worker:j.worker,product:j.product,machine:alloc[i].name,qty:alloc[i].qty,start:segStart,end:segEnd,groupStart:start,groupEnd:end,requestedStart:leader.start,groupId:groupIds.get(j.id),dry,lot:j.lot,manual:!!j.manual,produced:j.produced,plan:j.totalPlan,remaining:j.remaining,status:j.stopped?'중단':j.complete?(statusToday>=dry?'건조 완료':'건조 중'):!hasProduction(j)?'미착수':'생산중',projected:!j.complete&&!observedSpans?.[i]?.actualEnd,...(observedSpans?.[i]?.actualEnd?{machineComplete:true}:{}),description:j.legacyInput||'',source:j.sourceRowId?'생산일보':'웹 작업'})}}cursors[w]=!cursors[w]||cursors[w]<end?end:cursors[w]}
 ...
 const ownedRows=new Set(raw.flatMap(j=>(j.records||[]).map(r=>r.id))),externalJobs=new Map(),ownedByLaterTarget=r=>{const target=scheduleTargetOf(r);if(!target||target.month<=k||!s.months[target.month])return false;if(!externalJobs.has(target.month))externalJobs.set(target.month,jobs(s,target.month));const row=named(s,r),matches=externalJobs.get(target.month).filter(job=>recordMatchesJob(job,row,s));return matches.length===1};
 for(const r of options.isolatedGroup?[]:s.months[k].rows||[]){if(r.deleted||ownedRows.has(r.id)||ownedByLaterTarget(r)||!(C.num(r.pours)>0)||!C.iso(r.date)||!r.worker||!r.product||C.catalogIdentity(s,r.product).includes('부속'))continue;const id='actual:'+r.id,product=C.catalogName(s,r.product);output.push({id,jobId:id,sourceRowId:r.id,groupId:r.worker+'|'+id,worker:r.worker,product,machine:'미배정',qty:null,plan:null,produced:C.num(r.pours),remaining:null,start:r.date,end:r.date,groupStart:r.date,groupEnd:r.date,dry:null,lot:r.lot||'',manual:false,status:endsProduction(r)?'건조 중':'생산중',projected:false,actualOnly:true,orphanActual:true,source:'생산일보'});warnings.push({id:r.id,sourceRowId:r.id,product,message:'실제 생산일을 표시했습니다. 일보에서 시작 계획·작업 연결을 확인하면 호기와 예상 종료일이 연결됩니다.'})}
 ...
 for(const issue of options.isolatedGroup?[]:reservationContext(s,k).issues){const record=s.months[k].rows.find(r=>r.id===issue.rowId);warnings.push({id:issue.rowId,product:record?.product||'',message:issue.message})}
 const displayed=splitHandoffRows(options.skipSourceDry?output:preserveSourceDryDates(s,k,output,today),all),actualIds=new Set([...all.filter(hasProduction).map(j=>j.id),...output.filter(r=>r.orphanActual).map(r=>r.jobId)]),pendingIds=new Set(all.filter(j=>j.manual&&!hasProduction(j)&&!j.complete).map(j=>j.id)),conflictWarnings=new Set();
...
function scheduleAll(s,requestedToday,options={}){return C.withCatalogLookup(s,()=>withAssignmentCalendar(s,()=>scheduleAllRead(s,requestedToday,options)))}
function scheduleAllRead(s,requestedToday,options={}){
 ...
 for(const owner of months){base[owner]=scheduleBase(s,owner,requestedToday,settings(owner,{skipSourceDry:true}));if(s.months[owner].closed)continue;for(const job of base[owner].jobs||[])entries.push({owner,job,key:jobKey(owner,job.id)});effective.set(owner,new Map((base[owner].jobs||[]).map(job=>[job.id,job])))}
 ...
 for(const entry of selected){const key=entry.owner+'\0'+entry.job.worker+'\0'+(entry.job.handoffs?.length?'job:'+entry.job.id:productionGroupKey(entry.job));if(!groups.has(key))groups.set(key,{key,owner:entry.owner,worker:entry.job.worker,members:[],requestedStart:entry.job.start});const group=groups.get(key);group.members.push(entry);if((entry.job.start||'9999')<(group.requestedStart||'9999'))group.requestedStart=entry.job.start}
 ...
 for(const group of groups.values()){
  ...
  if(active){const incomplete=group.members.filter(e=>!e.job.complete),parts=group.members.flatMap(rowsFor),workers=new Set([group.worker,...parts.map(row=>row.worker),...incomplete.map(e=>workerAt(e.job,asOf(s,e.owner)))]);
   for(const worker of workers){const ownedParts=parts.filter(row=>row.worker===worker),current=cursors.get(worker),unknown=incomplete.some(e=>(e.job.carryBlocked||e.job.handoffBlocked||!rowsFor(e).some(row=>!row.actualOnly&&!row.reservationOnly&&C.iso(row.end)))&&workerAt(e.job,asOf(s,e.owner))===worker),ends=ownedParts.filter(row=>!row.actualOnly||row.handoffOut||group.members.find(e=>e.job.id===row.jobId)?.job.complete).map(row=>row.end).filter(C.iso).sort(),end=ends.at(-1)||null;
    for(const entry of incomplete.filter(e=>hasActual(e.job))){const owned=ownedMachineEvents(s,entry.owner,entry.job),start=owned.initial>0?null:owned.events.filter(event=>workerAt(entry.job,event.date)===worker).map(event=>event.date).filter(C.iso).sort()[0]||ownedParts.map(r=>r.start).sort()[0]||null,prior=activeStarts.get(worker);activeStarts.set(worker,!start||prior===null?null:prior&&prior<start?prior:start)}
    if(unknown||end&&!current?.blocked&&(!current||end>current.end))cursors.set(worker,{end,blocked:unknown,predecessor:predecessor(group,end),reason:unknown?'진행 중인 앞 작업의 예상 종료일을 확인해야 다음 수기 일정을 배치할 수 있습니다.':''});
   }
 ...
 for(const group of pending){const previous=cursors.get(group.worker),manualFallback=!!previous?.blocked,anchor=manualFallback?null:previous,status=manualFallback?'manual':previous?'chained':'first';let prepared=null,calculated=null,start=null,end=null,reason='';
  try{
   ...
   end=calculated.rows.map(row=>row.end).filter(C.iso).sort().at(-1);if(!end)throw Error('수기 작업의 생산 종료일을 계산할 수 없습니다.');
   for(const job of prepared)effective.get(group.owner).set(job.id,job);
  }catch(error){reason=error.message}
  for(const entry of group.members){const parts=reason?[]:calculated.rows.filter(row=>row.jobId===entry.job.id),starts=parts.map(row=>row.start).filter(C.iso).sort(),ends=parts.map(row=>row.end).filter(C.iso).sort();layouts.set(entry.key,{ownerMonth:entry.owner,jobId:entry.job.id,groupKey:group.key,worker:entry.job.worker,requestedStart:entry.job.start||null,requestedEnd:entry.job.end||null,start:starts[0]||null,end:ends.at(-1)||null,groupStart:reason?null:start,groupEnd:reason?null:end,status:reason?'blocked':status,gapWorkdays:2,predecessor:previous?.predecessor||null,reason:reason||(manualFallback?'앞 작업의 종료일이 정해지지 않아 저장한 수기 예약조건을 사용합니다.':'')})}
  cursors.set(group.worker,{end:reason?null:end,blocked:!!reason,predecessor:predecessor(group,reason?null:end),reason:reason||''});
 }
 for(const owner of months){
  if(s.months[owner].closed){result[owner]=base[owner];continue}
  const original=base[owner],jobs=(original.jobs||[]).map(job=>effective.get(owner).get(job.id)||job),calculated=scheduleBase(s,owner,requestedToday,settings(owner,{inputJobs:jobs})),monthLayouts=[...layouts.values()].filter(entry=>entry.ownerMonth===owner);
  calculated.jobs=original.jobs;calculated.reservationLayout={policy:reservationLayoutPolicy,entries:monthLayouts};calculated.rows=calculated.rows.filter(row=>!superseded.has(jobKey(owner,row.jobId)));
  for(const layout of monthLayouts){const sourceJob=original.jobs.find(job=>job.id===layout.jobId);let parts=calculated.rows.filter(row=>row.jobId===layout.jobId);
   for(const row of parts){row.reservationLayout=clone(layout);row.requestedStart=original.rows.find(old=>old.id===row.id)?.requestedStart||layout.requestedStart;if(layout.status==='blocked'){row.reservationOnly=true;row.reservationBlocked=true;row.dry=null;row.drySource='자동 배치 보류';row.status='배치 보류';row.description=layout.reason}}
   if(layout.status==='blocked')calculated.warnings.push({id:layout.jobId,product:sourceJob.product,message:layout.reason});
  }
  calculated.rows.sort((a,b)=>a.start.localeCompare(b.start));result[owner]=calculated;
 }
 return result;
}
function schedule(s,k,requestedToday,options={}){if(!s.months?.[k])throw Error('계획 월자료가 없습니다.');if(s.months[k].closed)return scheduleBase(s,k,requestedToday,options);return scheduleAll(s,requestedToday,{...options,asOfMonth:k})[k]}
// Read-only display projection. Daily ledger totals, inventory and job progress
// remain separate measures. Drawing one bar or separate machine bars is not an
// input: each physical job contributes once to its production group.
function timelineCumulative(s,k,result,first,last){return withAssignmentCalendar(s,()=>timelineCumulativeRead(s,k,result,first,last))}
function timelineCumulativeRead(s,k,result,first,last){
 if(!C.iso(first)||!C.iso(last)||first>last)return[];
 const output=[],groups=new Map(),workdays=new Map(),jobMap=new Map(),rowMap=new Map();
 const key=(owner,id)=>owner+'\0'+id,positive=v=>Number.isFinite(v)&&v>0,round=v=>Math.round(v*1000000)/1000000;
 const allowed=(worker,date)=>{const id=worker+'\0'+date;if(!workdays.has(id))workdays.set(id,manualWork(s,worker,date));return workdays.get(id)};
 for(const j of result.jobs||[])jobMap.set(key(j._ownerMonth||k,j.id),j);
 for(const row of result.rows||[]){if(!C.iso(row.start)||!C.iso(row.end))continue;const id=key(row.editMonth||row.sourceMonth||k,row.jobId);if(!rowMap.has(id))rowMap.set(id,[]);rowMap.get(id).push(row)}
 function series(j,owner,parts){
  const starts=parts.map(r=>r.explicitRouting?r.start:r.groupStart||r.start).sort(),ends=parts.map(r=>r.explicitRouting?r.end:r.groupEnd||r.end).sort(),start=starts[0],end=ends.at(-1),points=new Map(),basis=asOf(s,owner);
  const valid=(j.records||[]).filter(r=>!r.deleted&&C.iso(r.date)&&positive(r.pours)),ids=new Set(),duplicates=new Set();for(const r of valid)if(r.id){if(ids.has(r.id))duplicates.add(r.id);ids.add(r.id)}
  const clean=valid.filter(r=>!duplicates.has(r.id)),safeJob={...j,records:clean},owned=j.carried&&first<owner+'-01'?ownedMachineEvents(s,owner,safeJob):{initial:j.carried?(j.effectivePreviousProduced??j.previousProduced??0):0,events:clean};
  const daily=new Map();for(const event of owned.events)if(C.iso(event.date)&&positive(event.pours))daily.set(event.date,(daily.get(event.date)||0)+event.pours);
  let cumulative=Math.max(0,C.num(owned.initial)||0);for(const[date,qty]of[...daily].sort(([a],[b])=>a.localeCompare(b))){cumulative=round(cumulative+qty);points.set(date,{value:cumulative,kind:'actual'})}
  const uncertain=duplicates.size>0||valid.length!==(j.records||[]).filter(r=>!r.deleted&&r.pours!=null&&r.pours!==0).length;
  const active=daily.size>0||owned.initial>0,firstForecast=active?add([basis,...daily.keys()].sort().at(-1),1):j.manual||j.routing?start:[start,add(basis,1)].sort().at(-1);
  const warning=j.handoffReview|| (duplicates.size?'중복 실적 ID는 누적에서 제외했습니다. 원장 연결을 확인해 주세요.':uncertain?'유효한 날짜와 양수 실적만 누적했습니다. 입력을 확인해 주세요.':'');
  if(!j.complete&&!j.carryBlocked&&!j.handoffBlocked&&!uncertain&&parts.some(r=>!r.actualOnly&&!r.reservationOnly)&&firstForecast<=end){
   const explicit=!!j.routing,target=explicit?C.sum(machineQty(j),r=>r.qty):j.totalPlan;
   if(explicit){
    const allocation=machineQty(j),stats=allocation.map((m,i)=>{const r=parts.find(r=>(r.physicalRowId||r.id)===j.id+':'+i);return{row:r,remaining:r?Math.max(0,(r.qty||0)-(r.machineProduced||0)):0}});
    for(let date=firstForecast,n=0;date<=end&&n<740;date=add(date,1),n++){
     if(!allowed(workerAt(j,date),date))continue;let capacity=dailyAt(j,date),qty=0;
     for(const stat of stats){const r=stat.row;if(!r||r.actualOnly||r.machineComplete||date<(r.physicalStart||r.start)||date>(r.physicalEnd||r.end)||!(stat.remaining>0))continue;const rate=j.routing.mode==='parallel'?dailyAt(j,date,machineQty(j).find(m=>routingMachineName(m.name)===r.machine)):capacity,part=Math.min(stat.remaining,rate);if(!positive(part))continue;stat.remaining=round(stat.remaining-part);qty+=part;if(j.routing.mode==='sequential')capacity-=part}
     if(qty>0){cumulative=round(cumulative+qty);points.set(date,{value:cumulative,kind:'forecast'})}
    }
   }else if(positive(j.daily)&&positive(target)){
    for(let date=firstForecast,n=0;date<=end&&n<740&&cumulative<target;date=add(date,1),n++){if(!allowed(workerAt(j,date),date))continue;cumulative=round(Math.min(target,cumulative+dailyAt(j,date)));points.set(date,{value:cumulative,kind:'forecast'})}
   }
  }
  return{job:j,owner,parts,start,end,points,basis,warning};
 }
 for(const[id,parts]of rowMap){const j=jobMap.get(id);if(!j||!j.worker)continue;const owner=j._ownerMonth||k,entry=series(j,owner,parts),group=j.handoffs?.length?'job:'+j.id:productionGroupKey(j,parts[0]),gkey=key(owner,j.worker+'\0'+group);if(!groups.has(gkey))groups.set(gkey,[]);groups.get(gkey).push(entry)}
 const pointEntry=(group,entry,date,point,members)=>({worker:workerAt(entry.job,date),date,value:point.value,kind:point.kind,product:entry.job.product,jobId:entry.job.id,groupId:group,plan:entry.job.totalPlan,basis:entry.basis,warning:entry.warning,members:members.map(m=>({product:m.job.product,jobId:m.job.id,value:m.points.get(date)?.value??null,kind:m.points.get(date)?.kind||null}))});
 for(const[group,members]of groups){
  let selected;
  ...
  for(const entry of members)for(const[date,point]of entry.points){if(date<first||date>last)continue;const clipped=entry.parts.some(r=>Array.isArray(r.displaySegments));if(point.kind==='forecast'&&clipped&&!entry.parts.some(r=>(r.displaySegments||[{start:r.start,end:r.end}]).some(span=>date>=span.start&&date<=span.end)))continue;output.push({...pointEntry(group,entry,date,point,members),pacing:selected.includes(entry)})}
 }
 // Historical Excel cells remain explicitly saved values. Never manufacture
 // daily detail from a closed aggregate or overlay stale cells on live jobs.
 const grid=(s.archive||[]).find(x=>x.name==='생산일정(계획)'),year=C.num(C.val(grid,'D1'));
 if(grid&&Number.isInteger(year)){
  const live=[...groups.values()].flat(),leap=year%4===0&&(year%100!==0||year%400===0),savedRows=(result.rows||[]).filter(r=>r.history&&/^생산일정\(계획\)!/.test(r.source||''));
  for(const r of savedRows){const match=/!([A-Z]+)(\d+)$/.exec(r.source);if(!match)continue;const numericRow=Number(match[2])+3;
   for(let date=r.start;date<=r.end;date=add(date,1)){if(date<first||date>last||live.some(m=>m.job.worker===r.worker&&date>=m.start&&date<=m.end))continue;const offset=Math.round((Date.parse(date)-Date.UTC(year,0,1))/86400000),column=6+offset+(!leap&&offset>=59?1:0),value=C.num(C.val(grid,C.col(column)+numericRow));if(!positive(value))continue;output.push({worker:r.worker,date,value,kind:'saved',product:r.product,jobId:r.jobId,groupId:r.groupId||r.jobId,plan:null,basis:null,warning:'엑셀에 보관된 작업 누적입니다. 현재 원장으로 재계산한 실적이 아닙니다.',members:[]})}
  }
 }
 return output.sort((a,b)=>a.date.localeCompare(b.date)||a.worker.localeCompare(b.worker)||a.groupId.localeCompare(b.groupId));
}
