...
function scheduleTargetValue(value){
 ...
 const fields=['jobId','originId','month','worker','productId'];
 if(!value||typeof value!=='object'||Array.isArray(value)||value.schema!==1||fields.some(k=>typeof value[k]!=='string'||!value[k].trim()||value[k]!==value[k].trim()||value[k].length>200||/[\x00-\x1f\x7f]/.test(value[k]))||!/^\d{4}-\d{2}$/.test(value.month)||!iso(value.month+'-01'))throw Error('생산량을 반영할 작업 연결을 다시 확인해 주세요.');
 return{schema:1,...Object.fromEntries(fields.map(k=>[k,value[k]]))};
}
...
function productionTeamValue(value,owner=''){
 if(value==null)return null;
 if(value.schema!==1||!Array.isArray(value.members)||value.members.length>1)throw Error('함께 작업한 사람을 다시 선택해 주세요.');
 const seen=new Set([owner.replace(/\s+/g,'')]);
 const members=value.members.map(m=>{const worker=typeof m?.worker==='string'?m.worker.replace(/\s+/g,''):'';
