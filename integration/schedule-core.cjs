...
function catalogProduct(state,name){const key=text(name);if(!key)return null;const matches=(state.products||[]).filter(p=>text(p.name)===key||(p.renameFrom||[]).some(old=>text(old)===key));if(matches.length>1)throw Error('품명 변경 이력이 다른 품목과 중복됩니다: '+key);return matches[0]||null}
...
function inventory(state,key){const m=state.months[key];if(!m)return[];if(m.closed&&m.closeSnapshot?.inventory)return clone(m.closeSnapshot.inventory);const name=v=>m.closed?text(v):catalogName(state,v),previous=m.openingSource&&m.openingSource<key?inventory(state,m.openingSource):[],names=new Set([...m.stocks,...m.rows,...m.issues,...previous].map(r=>name(r.product)).filter(Boolean));return [...names].map(product=>{const ss=m.stocks.filter(s=>name(s.product)===product),previousRows=previous.filter(p=>name(p.product)===product);if(!m.closed&&previousRows.length>1)throw Error('이전 월의 품명 연결이 중복됩니다: '+product);const carried=previousRows[0]?.current;const opening=ss.length?sum(ss,s=>s.sourceType==='previous'?(carried??(state.months[m.openingSource]?0:s.opening)):s.opening):(carried??0);const production=sum(m.rows.filter(r=>name(r.product)===product),r=>r.pours);const deduction=sum(m.issues.filter(r=>name(r.product)===product),r=>r.replacement);const current=opening+production-deduction,kg=state.products.find(p=>p.name===catalogName(state,product))?.kg;return{product,opening,production,deduction,current,kg:kg==null?null:current*kg,original:ss.some(s=>s.current!=null)?sum(ss,s=>s.current):null,source:ss.map(s=>s.source).join(', ')}})}
...
root.ScheduleCore={workerStats,text,num,col,ci,val,date,serial,iso,clone,id,catalogName,catalogIdentity,renameCatalogReferences,daily,dailyQuality,dryDate,sum,managementProducts,catalogUnitSources,refreshCatalogUnits,scheduleSourceDate,savedSchedulePeriods,savedScheduleDryMarkers,normalize,inventory,completionValue,validateRecord,updateDaily};
